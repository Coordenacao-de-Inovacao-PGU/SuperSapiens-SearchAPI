...
class SuperSapiensService():
    ...
    def prepare_params(self, **kwargs):
        ...
        headers['X-Rate-Limit-API'] = 'optimus'
    ...
    def login(self, username: str, password: str) -> dict:
        ...
        if 'token' in resp_body:
            ...
            result['success'] = True
            retorno = resp_body['code']

        if result.get("success", False):
            return {
                "status": resp_body['code'],
    ...
    def search_sector(self, query):
        ...
        querystring = {
            "where":
                "{\"parent\":\"isNotNull\","
                "\"unidade.id\":\"eq:"+self.unity+"\","
                "\"orX\":[{\"andX\":"
                "[{\"nome\":\"like:%"+query+"%\"}]},"
                "{\"andX\":[{\"sigla\":\"like:%"+query+"%\"}]}]}",
    ...
    def search_all(self, content, extension, unity,  document_type,  created_at, created_on, created_by, sector):
        ...
        querystring = {
            "where": "{\"andX\":[{\"conteudo\":\"like:%"+content+"%\"},"
                     "{\"extensao\":\"like:%"+extension+"%\"},"
                     "{\"documento.tipoDocumento.id\":\"eq:"+self.document_id+"\"},"
                     "{\"criadoEm\":\"gte:"+created_at+"\"},"
                     "{\"criadoEm\":\"lte:"+created_on+"\"},"
                     "{\"criadoPor.id\":\"eq:"+self.user_id_searched+"\"},"
                     "{\"documento.setorOrigem.id\":\"eq:"+self.sector_id+"\"}]}",
